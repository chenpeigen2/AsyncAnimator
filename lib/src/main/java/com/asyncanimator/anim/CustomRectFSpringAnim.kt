...
class CustomRectFSpringAnim internal constructor(
     ...
     */
    @PublicApi
    data class Event @PublicApi constructor(
        /** 启动时固定的动画标识；后续运行不会改写已派发事件。 */
        @property:PublicApi
        val animationId: Int,
        /** 句柄内部本轮运行代次；用于区分重启前后的生命周期通知，不是全局唯一编号。 */
        @property:PublicApi
        val runId: Long,
        /** 通知创建时本轮是否已取消；不可变快照，不随后续状态变化。 */
        @property:PublicApi
    ...
    @PublicApi
    enum class AnimType {
        /** 从桌面打开应用的场景分类；用于参数策略，不自动启动系统窗口转场。 */
        @PublicApi
        OPEN_FROM_HOME,
        /** 远程关闭回桌面的场景分类；实际几何及窗口操作仍由宿主驱动提供。 */
        @PublicApi
        REMOTE_CLOSE_TO_HOME,
        /** 远程关闭回桌面的辅助场景分类；不隐式创建远程动画目标。 */
        @PublicApi
        REMOTE_CLOSE_TO_HOME_ASSISTANT,
        /** 手势转入拖拽的场景分类；六轴配置保留原始刚度，不应用业务时长倍率缩放。 */
        @PublicApi
        GESTURE_TO_DRAG,
        /** 滑动回桌面的场景分类；驱动能力与线程由显式实现决定。 */
        @PublicApi
        SWIPE_TO_HOME,
        /** 滑动回桌面的辅助场景分类；本标记不提供额外平台集成。 */
        @PublicApi
        SWIPE_TO_HOME_ASSISTANT,
        /** 反转为打开应用的场景分类；是否支持重定向由具体驱动声明。 */
        @PublicApi
        REVERSE_TO_OPEN
    }
