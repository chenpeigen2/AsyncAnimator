...
|---|---|---|
| `control/AnimationController.kt`（259 行） | `com/oplus/quickstep/utils/AnimationController.java:58`（`extends DefaultAnimationController`，≈990 行，Kotlin 反编译 `classes5.dex`） | 精确对应；lib 是 Impl、保留所有原厂关键 API |
| `control/AnimationState.kt:11-27`（12 值） | `AnimationController.java:103-115` `enum AnimationState`（12 值，私有） | 12 状态 + `withTaskbarAlignment`/`taskbarAlignmentToLauncher` 双 boolean 取值逐一相同；lib 提升为顶层枚举 |
| `control/DefaultAnimationController.kt:13`（open class，no-op 基类） | `com/oplus/quickstep/utils/DefaultAnimationController.java:30`（`public class DefaultAnimationController`） | 精确对应（no-op 基类），是 Ext 模式"feature off 时业务调用安全"语义本体 |
| `control/OnAnimStateChangeListener.kt:11`（fun interface） | `DefaultAnimationController.java:34-36` `interface OnAnimStateChangeListener` | 形状等价；lib 第三参 `Any?`、原厂为 `TaskInfo`（`DefaultAnimationController.java:35`） |
| `control/TaskStateChangeTimeOutListener.kt:12-47`（class + 自管理超时；非 fun interface） | `com/oplus/quickstep/taskviewremoteanim/TaskStateHelper.java:117-211` `static class TaskStateChangeTimeOutListener extends BaseTaskStateChangeListener` | **仅名字 + 意图对应，机制严重偏离**（见 §2.2-4、§3-d） |
| `control/RemoteAnimationFactory.kt:10-17`（lib 自定义 2 方法 demo 接口） | `com/android/launcher3/LauncherAnimationRunner.java:242-275` `public interface RemoteAnimationFactory`（FunctionalInterface，10 个 default 方法 + 1 abstract） | lib 是 demo 重写，原厂含 `onCreateAnimation` + `appLaunchAnimStartOrEnd`/`getAnimation`/`handleAnimationMerged`/`isSameIcon`/`onAnimationCancelled`/`preLoadIcon`/`supportInterruption`/`tryFinishOpenRemote`/`getIconSurfaceRecordId` |
| `manager/OplusAnimManager.kt:17`（object 单例） | `com/oplus/quickstep/utils/OplusAnimManager.java:20`（`public final class OplusAnimManager`，Kotlin `object` 反编译） | 对应；原厂管 6 个 helper，lib 只出 2 个（`animController` + `animationSeqHelper`） |
| `seq/AnimationSeqHelper.kt:27` | `com/oplus/quickstep/utils/AnimationSeqHelper.java:17`（`extends DefaultAnimationSeqHelper`） | 精确对应 |
| `seq/DefaultAnimationSeqHelper.kt:8` | `com/oplus/quickstep/utils/DefaultAnimationSeqHelper.java:10` | 精确对应（no-op 基类） |
| `seq/AnimSeqTimeStamp.kt:11`（object，4 字段 volatile） | `com/android/systemui/shared/system/AnimSeqTimeStamp.java:10`（`public final class`，`@JvmStatic synchronized` 4 字段） | 对应；lib 用 Kotlin `object`+`@Volatile`，原厂用 Java 类 + 静态方法 |
| `manager/AnimationFeatureHelper.kt:13`（object + 7 @Volatile 字段 + 2 只读列表） | `com/oplus/quickstep/utils/AnimationFeatureHelper.java:25`（`public final class`，6 配置项 + 2 列表 + RUS 远程下发） | 对应，lib 是本地 setter 模拟 RUS |
| `com/android/launcher3/LauncherAnimationRunner.kt:13-19`（21 行桩，保留 `RemoteAnimationTarget` 壳） | `com/android/launcher3/LauncherAnimationRunner.java`（≈600 行，`extends RemoteAnimationRunnerCompat`） | 仅类型壳对应；lib 文件头注释明示 |
...
| 2 | **AnimationFeatureHelper 用本地 setter 模拟 RUS 下发** | `AnimationFeatureHelper.java:82-92, 169-373`：`RusBaseConfigManager.RusConfigChangedListener` 回调 + `updateRusConfig()` 解析 7 配置项（`launcher_anim_prj_feature_async_enable` / `..._rt_unlock_enable` / `..._1px_enable` / `..._icon_blur_enable` / `..._multi_app_block_enable` / `launcher_anim_interrupt_threshold` / `launcher_trigger_pre_boot_limit_size`）+ 2 列表（pkg / card） | lib `manager/AnimationFeatureHelper.kt:47-66` `simulateRemoteUpdate`（9 参全量版 :47-60 + 6 参兼容版 :63-66，086844e）覆盖"配置可变"语义；同步 + volatile 机制保留 |
...
| 5 | **OnAnimStateChangeListener 改 fun interface + 快照迭代** | `DefaultAnimationController.java:155-162` 内部 `Iterator` 直接遍历 `animStateChangeListeners`（遍历中 add/remove 触发 CME） | lib `control/OnAnimStateChangeListener.kt:11-14` fun interface（非 typealias，保证引用相等可 remove，见 :8-9 注释）+ `control/DefaultAnimationController.kt:27` `for (l in animStateChangeListeners.toList())` 拷贝快照（lib 注释明示更安全） |
| 6 | **AnimSeqTimeStamp 的 reset 收进 `resetAllForTest`** | `AnimSeqTimeStamp.java:69-107` 4 个 `@JvmStatic synchronized` public `resetLast*Time()` + `Log.d` | lib `seq/AnimSeqTimeStamp.kt:45-67` 全部 `internal resetLast*Time()`（:45-59）+ `internal resetAllForTest`（:62-67），主入口仅暴露 `updateLastRecentFinishTime()`（:33） |
...
| 9 | **APP_TO_OVERVIEW_CONTINUATION_TIME_OUT_DURATION = 100ms** | `AnimationController.java:59` 同名常量 | lib `control/AnimationController.kt:8` 同样取 100ms（直接保留） |
...
| # | 遗漏 | 原厂证据 | 风险定性 |
|---|---|---|---|
| 1 | **`appLaunchAnimStartOrEnd` 的 end 分支 + start 分支的 `mAppLaunchAnims.add(factory)`** | `AnimationController.java:508-556` end 时 `mHandler.removeMessages(101)` → `sendEmptyMessage(101)` → `mAppLaunchAnims.remove(factory)`；空列表 + `!mOnceGestureProcessing` → `checkAllAnimationFinished()`；否则按 `OPEN`/`MULTI_OPEN` 转 `WAITING`/`MULTI_WAITING`。start 分支在 `:547` 显式 `mAppLaunchAnims.add(appLaunchAnimFactory)`。**（旧版描述，60bd048 已落地 end 分支）** | **✅已修复（cdd125e+60bd048，见 §3-a）**：end 分支 + `appLaunchAnims.add` 已实现（`AnimationController.kt:100-124`），OPEN→WAITING / MULTI_OPEN→MULTI_WAITING 可转移；仍缺 `MESSAGE_RELEASE_TOUCH`(101) 600ms 闸门与 `mOpenWindowAnimRunning`（见 §2.3-5、§3-f） |
| 2 | **`checkAllAnimationFinished` 收尾通路 + 两个 callback 执行** | `AnimationController.java:224-231, 237-249, 251-263, 309-320, 344-360, 382-395`：`checkAllAnimationFinished` 内联在 `cleanUpRecentsAnim` 路径调 `executeRemoteMergeFinishCallback` + `executeRecentMainFinishCallback` → `reset()`；非主线程时 `MAIN_EXECUTOR.execute`/`UI_HELPER_EXECUTOR.execute` 纠偏 | lib `AnimationController.kt:50-51, 126-132`（两个 callback 字段 + `checkAllAnimationFinished` 调用它们并 `reset()`；触发点 `cleanUpRecentsAnim` :88、`appLaunchAnimStartOrEnd` end 分支 :112）——**✅已修复（60bd048）** |
| 3 | **`revertRecentsAnimation`** | `AnimationController.java:825-837`：`mCurrentAnim = anim` + 按 `CLOSE`→`REVERSE_OPEN` / `MULTI_CLOSE`→`MULTI_REVERSE_OPEN` / 其它→`UNKNOWN` 转移 | lib `AnimationController.kt:92-98` 已 override（CLOSE→REVERSE_OPEN / MULTI_CLOSE→MULTI_REVERSE_OPEN / 其它→UNKNOWN）——**✅已修复（60bd048）**，两状态现可达 |
...
| 6 | **`isStartActivityBetweenTransitionEndAndFinish` 判定不全** | `AnimationController.java:798-799`：`mIsBetweenTransitionEndAndFinish && startActivityRunnable != null`（双条件） | lib `AnimationController.kt:205-206` 只读 `isBetweenTransitionEndAndFinish` 字段，**且 `setBetweenTransitionEndAndFinish`/`setBetweenAppExitTransitionEndAndFinish` 走基类 no-op**（`control/DefaultAnimationController.kt:66-67`），`isBetweenTransitionEndAndFinish` 永为 false；该方法在 lib 恒 false |
| 7 | **`setOnAppExit` 的场景判定体** | `AnimationController.java:881-895`：`NavigationMode == THREE_BUTTONS` && `!isLargeDisplayDeviceInLarge() && isDeviceInLandscape() && LauncherAnimConfig.isAdaptiveLowAnimation()` 三重门槛；满足才 `mIsNavModeLandScapeOnAppExit = true` + `registerSpecialSceneExitTimeOutListener(1500L)` | lib `AnimationController.kt:204-208` **无条件置三个 flag 为 true**（包括 `isNavModeLandScapeOnAppExit`） |
| 8 | **`setOnceGestureProcessing` 的完整判定体** | `AnimationController.java:898-952`：`mSwipingUpActivityPkg` 从 `OplusGestureState` 提取（baseActivity/topActivity）；`getIsLandScape` / `getIsContinuationHandling` / `TopTaskTracker.runningSplitTaskIds` 三源；按 `isSplitScreen\|\|isRecentContinuation` 分支 → `registerSpecialSceneExitTimeOutListener(1500L)` 或 `registerTransitionFinishListener(1500L)`；null 走 `mIsSplitScreenGesture && mIsLandScapeGesture && !isTablet && isHomeAndOverviewSame → registerSpecialSceneExitTimeOutListener(LAND_SPACE_RECENT_ANIM_TIME_OUT_DURATION=2500L)` | lib `AnimationController.kt:210-212` **只置 `onceGestureProcessingFlag = true`**，不读 `OplusGestureState`、不注册 listener |
...
| 11 | **`resetInterceptState()` 的 override** | `AnimationSeqHelper.java:111-114`：调 `AnimSeqTimeStamp.resetLastStartAppTime()` | **✅已修复（60bd048）**：`seq/AnimationSeqHelper.kt:86-88` 已 override 并调 `seq/AnimSeqTimeStamp.resetLastStartAppTime()`（`AnimSeqTimeStamp.kt:45-47`） |
...
**a.（高 / bug 级→已修复）`appLaunchAnimStartOrEnd` end 分支 + `mAppLaunchAnims.add` 链路**（§2.3-1）。当前实现（`AnimationController.kt:100-124`）：start 分支 `factory?.let { appLaunchAnims.add(it) }`（`:116`）+ `NONE→OPEN` / `CLOSE`、`MULTI_CLOSE→MULTI_OPEN` / 其它→`UNKNOWN`（`:117-122`）；end 分支先 `appLaunchAnims.remove(factory)`（`:103`），列表空时按 `onceGestureProcessingFlag` 走 `OPEN→WAITING`/`MULTI_OPEN→MULTI_WAITING`（`:105-110`）或 `checkAllAnimationFinished()`（`:112`）。`checkAllAnimationFinished`（`:126-132`）执行两个 callback 并 `reset()`。残余缺口：原厂 `MESSAGE_RELEASE_TOUCH(101)` 600ms 闸门与 `mOpenWindowAnimRunning`（`forbidTouch()` 输入防抖）仍未移植（见 §2.3-5/§3-f）；callback 的 `MAIN_EXECUTOR/UI_HELPER_EXECUTOR` 线程纠偏也未复刻（见 §3-n）。
...
**i.（中）`setOnAppExit` 无条件置位**（§2.3-7）。原厂三键导航 + 横屏 + 低档机动画模式三重门槛（`:885-892`）；lib `AnimationController.kt:204-208` 无条件 `isLandScapeGesture = true`、`isNavModeLandScapeOnAppExit = true`、`isBetweenAppExitTransitionEndAndFinish = true`。第一层决策树（`delayStartActivityIfNeed` 第一分支）的命中条件 `isLandScapeGesture || isSplitScreenGesture || (isNavModeLandScapeOnAppExit && isBetweenAppExitTransitionEndAndFinish)` 在 lib 里**永远命中**——任何进入该方法的调用都会进入"横屏退出"挂起路径，行为偏离原厂。
...
**k.（中）`OplusExecutors` 时钟域**。lib 全部用 `SystemClock.uptimeMillis()`（`AnimationController.kt:181, 199, 225, 237` / `seq/AnimationSeqHelper.kt:76` / `seq/AnimSeqTimeStamp.kt:27`）；原厂同样 `SystemClock.uptimeMillis()`（`AnimationController.java:298, 334, 609` / `AnimationSeqHelper.java:96`）。**此项 lib 与原厂对齐**，但要注意 100/300/500/1500ms 窗口在"主线程被 binder 阻塞 / `UX_TASK_EXECUTOR` 离线事务饿死"时两边的"窗口计时"会不同——原厂超时跑在 `URGENT_TRANSACTION_EXECUTOR`（独立线程），lib 跑在主线程（`TaskStateChangeTimeOutListener.kt:31-32`）。

**l.（中）`reset()` 字段清理不全**。原厂 `AnimationController.java:811-822` 清 8 项（callbacks ×2、`mSwipingUpActivityPkg`、appLaunchAnims、recentsAnims、removeTasksMaps、mClickAppView、mCurrentAnim + `updateAnimState(NONE)`）。lib `AnimationController.kt:134-148` 清 11 项但**多清**几个 lib 自加的 boolean（`isLandScapeGesture`/`isSplitScreenGesture`/`isNavModeLandScapeOnAppExit`/`isBetweenAppExitTransitionEndAndFinish`/`isBetweenTransitionEndAndFinish`），**少清** `mSwipingUpActivityPkg`（lib 无此字段）和 `mCurrentAnim`（无），并漏原厂 call 顺序里"先 updateAnimState 再清 callback"的次序（lib 是先清 callback 再 updateAnimState，对 listener 通知而言**先清 callback 再发 NONE 转移**，原厂**先发 NONE 转移再清 callback**——lib listener 收到 NONE 时 callback 已被清，业务侧 `setRecentsAnimFinishCallback` 之后再 setState(NONE) 仍能正常接收，行为接近但不等价）。

**m.（中）`OplusAnimManager.cleanUpRecentsAnimation` 简化**。原厂 `OplusAnimManager.java:173-181` 在 `getMAnimationController().cleanUpRecentsAnim() == true` 时调 `getMMultiAppAnimMergeHelper().setOnTaskAppearedTarget(null)` + `getMAppOpenAnimMergeHelper().cleanUpRecentsAnim()`；lib `manager/OplusAnimManager.kt:48-50` 只调 `animationControllerImpl?.cleanUpRecentsAnim()`。multi-app merge helper 在 lib 整体缺失。
...
**o.（低）`OnAnimStateChangeListener` 第三参类型弱化**。原厂 `DefaultAnimationController.java:35` 第三参 `TaskInfo`；lib `control/OnAnimStateChangeListener.kt:13` 第三参 `Any?`。**含义**：原厂 listener 可直接读 `taskInfo.taskId` / `taskInfo.baseActivity`，lib listener 要自己 `as? TaskInfo` 强转。对 demo 不影响。

**p.（低→已对齐）`getNextFinishSeqId` 的相等判定**。原厂 `AnimationSeqHelper.java:105, 124` 用 `Intrinsics.areEqual(...)`（结构相等）。lib 当前 `seq/AnimationSeqHelper.kt:90-103` 用 `p.first ==/!= recentsController`（`:93, :101`；Kotlin `==` 与 `Intrinsics.areEqual` 同义，60bd048 由旧 `===` 改为 `==`，`updateNextFinishSeqIdIfNeed` 同步改为"pair 为空或 controller 变更才更新" `:90-96`）——**引用 vs 结构相等的分歧已消除**。残留瑕疵（lib 注释，不改码）：`:100` 注释自称"这里用 === 保持一致"，与代码（`==`）不符，且"原厂按引用比较"表述有误。

**q.（低→已修复）`revertRecentsAnimation` 已实现**（§2.3-3）。lib `AnimationController.kt:92-98` 已 override（CLOSE→REVERSE_OPEN / MULTI_CLOSE→MULTI_REVERSE_OPEN / 其它→UNKNOWN，60bd048），`REVERSE_OPEN`/`MULTI_REVERSE_OPEN` 现可达；`isOpeningAnim`（`:150-154`）等依赖这两个状态的查询可覆盖完整路径。
...
1. ✅部分已落地（cdd125e+60bd048）——**`appLaunchAnimStartOrEnd` end 分支 + `mAppLaunchAnims.add(factory)` 已实现**（`AnimationController.kt:100-124`，对应 §2.3-1/§3-a）；**仍剩 `MESSAGE_RELEASE_TOUCH(101)` 600ms 闸门**（§2.3-5/§3-f）：需 `mHandler = Handler(Looper.getMainLooper())` + `mOpenWindowAnimRunning: Boolean` + `forbidTouch()` override（`mOpenWindowAnimRunning || animState==MULTI_WAITING || animState==REVERSE_OPEN || startActivityAction != null`）。
...
| 论断 | 证据 |
|---|---|
| lib `AnimationController` 12 状态 + 字段 | `D:/AsyncAnimator/lib/src/main/java/com/asyncanimator/control/AnimationController.kt:23-53` + `AnimationState.kt:11-27` |
| lib `addRecentsAnim` 转移表 | `AnimationController.kt:65-77` |
| lib `appLaunchAnimStartOrEnd` end 分支 + `mAppLaunchAnims.add`（已实现） | `AnimationController.kt:100-124`（start `:116` add；end `:102-114`） |
| lib `delayStartActivityIfNeed` 三层判定 | `AnimationController.kt:220-251`（`else if` 互斥 + 清理段 `:242-249`；仍缺 tablet/special-app/运行态判定） |
| lib `TaskStateChangeTimeOutListener` 是自管理超时 class（非 fun interface） | `D:/AsyncAnimator/lib/src/main/java/com/asyncanimator/control/TaskStateChangeTimeOutListener.kt:12-47` |
| lib `isStartActivityBetweenTransitionEndAndFinish` 恒 false | `AnimationController.kt:256-257` + `control/DefaultAnimationController.kt:66-67`（基类 no-op） |
| lib `setOnAppExit` 无条件置位 | `AnimationController.kt:204-208` |
| lib `setOnceGestureProcessing` 缩为一行 | `AnimationController.kt:210-212` |
| lib `AnimationSeqHelper` 三段与漏两段 | `seq/AnimationSeqHelper.kt:61-65` |
| lib `AnimationFeatureHelper` 默认 1/0 非 -1 | `manager/AnimationFeatureHelper.kt:17-29` |
| lib `OplusAnimManager` 只出 2 个 helper | `manager/OplusAnimManager.kt:20-30` |
...
> 本条为**独立逐条复核**（对照 HEAD 086844e 代码逐条验证，不采信上文 commit 交叉索引标记）。复核范围 = §1 类对应表 11 行 + §2.1 精确复刻 10 + §2.2 有意简化 10 + §2.3 遗漏 19 + §3 风险 a–q 17 + §4.1 回移建议 10 + §4.2 保持简化 8 = **85 条**；全部条目逐条对照 `control/`、`manager/`、`seq/`、`com/android/launcher3/` 当前代码，必要处 Grep 核对 OPPO 源码（AnimationState 12 态 boolean、AnimationSeqHelper `Intrinsics.areEqual`、AnimationFeatureHelper 默认 -1 等均已复核）。
>
> - **复核条目总数**：85
> - **结论不变**：69
> - **修正**：16
> - **修正明细**：
>   1. §1 OnAnimStateChangeListener 行：旧“typealias”→新“fun interface”（`control/OnAnimStateChangeListener.kt:11-14`；防 remove 失效）。
>   2. §1 TaskStateChangeTimeOutListener 行：旧“fun interface + 自管理超时”→新“class + 自管理超时，非 fun interface”（`control/TaskStateChangeTimeOutListener.kt:12-47`）。
>   3. §2.2-5：旧“改 typealias”→新“改 fun interface”（同上证据；快照迭代在 `control/DefaultAnimationController.kt:27`）。
>   4. §2.2-2：旧“`simulateRemoteUpdate` 一函数”→新“9 参全量版 + 6 参兼容版（086844e）”（`manager/AnimationFeatureHelper.kt:47-66`）。
>   5. §2.3-1：旧“bug 级（end 分支 + add 缺失）”→新“✅已修复（cdd125e+60bd048），见 §3-a”（`AnimationController.kt:100-124`）。
>   6. §2.3-2：旧“两个 callback 没有任何地方调用”→新“✅已修复（checkAllAnimationFinished 调用并 reset）”（`AnimationController.kt:126-132`，触发 `:88/:112`）。
>   7. §2.3-3：旧“`AnimationController` 未 override，两状态不可达”→新“✅已修复（60bd048）”（`AnimationController.kt:92-98`）。
>   8. §2.3-11：旧“`AnimationSeqHelper` 未 override”→新“✅已修复（60bd048）”（`seq/AnimationSeqHelper.kt:86-88` → `AnimSeqTimeStamp.resetLastStartAppTime()`）。
>   9. §3-a：正文旧描述“链路完全缺失”→新“已实现（end/add/checkAllAnimationFinished），残余 600ms 闸门与线程纠偏缺口”（`AnimationController.kt:100-132`）。
>   10. §3-b：正文旧描述“两处偏差”→新“已对齐（UNKNOWN→UNKNOWN、MULTI_WAITING→MULTI_CLOSE）”（`AnimationController.kt:65-77`）。
>   11. §3-c：正文旧描述“非互斥 + 无清理段”→新“✅部分修复（else-if 互斥 + 清理段已落地，仍缺 tablet/isSpecialAppScene/运行态判定）”（`AnimationController.kt:220-251`）。
>   12. §3-d：旧“fun interface”→新“自管理超时 class（非 fun interface/非 BaseTaskStateChangeListener）”+ 行号（`TaskStateChangeTimeOutListener.kt:12-47, 24, 30-32`；`delayStartActivityIfNeed :220-251`）。结论（仅 timeout 兜底、无全局事件总线）不变。
>   13. §3-g：描述更新“lib 默认 1/0（`limtSize=-1`/`interruptThreshold=1.0f` 与原厂一致）”+ 行号（`manager/AnimationFeatureHelper.kt:17-29`）。结论（5 个 int 无 -1 三态）不变。
>   14. §3-p：旧“lib 用 ===（引用相等）”→新“已对齐（60bd048 改 `==` ≡ `Intrinsics.areEqual`）”（`seq/AnimationSeqHelper.kt:90-103`）；残留 lib 注释 mislabel（:100）不改码。
>   15. §3-q：旧“revertRecentsAnimation 不可达”→新“已修复（60bd048）”（`AnimationController.kt:92-98`）。
